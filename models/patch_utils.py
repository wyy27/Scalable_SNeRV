...
def compute_paddings(output_patchsize=(1, 120, 120), scales=((1, 5, 5), (1, 4, 4), (1, 3, 3), (1, 2, 2)),
    ...
    """
    # print("output_patchsize:", output_patchsize)
    # print("scales:", scales)
    ...
    # print("paddings_reversed:", paddings_reversed)
    return list(reversed(paddings_reversed))

def upscale_resolution(img_tensor, scale):
    img_tensor = img_tensor.squeeze(1).permute(0, 3, 1, 2).contiguous()   # N, C, H, W
    _, _, H, W = img_tensor.shape
    img_tensor = F.interpolate(img_tensor, size=(scale[1]*H, scale[2]*W), mode="bicubic")
    img_tensor = img_tensor.unsqueeze(1).permute(0, 1, 3, 4, 2)    # N, T, H, W, C
    return img_tensor