...
class SNeRVEncoding(nn.Module):
    ...
    def __init__(self, size, channels, **kwargs):
        ...
        T, H, W = self.size    # encoding size t,h,w
        C = self.channels
    ...
    def forward(self, idx: torch.IntTensor, idx_max: tuple[int, int, int], padding: tuple[int, int, int]):
        ...
        # print("Froward HineRVEncoding:")
        # print("size", self.size)  #(600, 18, 32)
        # print("idx:", idx)   # tensor([[0, 0, 0]])
        # print("idx_max:", idx_max)  # (600, 1, 1)   # (600, 9, 16)
        # print("padding:", padding)  # (0, 0, 0)     # (0, 3, 3)
        ...
        # print("patch_size:", patch_size)   # (1, 18, 32)                 # (1, 2, 2)
        # print("patch_padded_size:", patch_padded_size)   # (1, 18, 32)   # (1, 8, 8)
        ...
        # print("px_idx:", px_idx)  # [tensor([[0]]), tensor([[ 0,  1,  2,  ... 17]]), tensor([[ 0,  1,  2,  3,  4,  5,  ... 26, 27, 28, 29, 30, 31]])]
        # print("px_idx_flat:", px_idx_flat) # tensor([[ 0,  1,  2,  3,  4,  5,  ... 575]])]     1*18*32
        # print("px_mask:", px_mask)  # [tensor([[True]]), tensor([[18 * ...True]]),   tensor([[32 * ...True]])
        # print("px_mask_flat:", px_mask_flat)
        ...
        # print("self.grid_level:", self.grid_level)  #  2
        # print("self.grid_sizes:", self.grid_sizes)   #  [(150, 18, 32, 2), (75, 18, 32, 4)]
        # print("enc_splits:", len(enc_splits))
        # print("enc:", enc.shape)  # torch.Size([600, 18, 32, 6])
        ...
        # print("output:", output.shape) # torch.Size([1, 1, 18, 32, 6])   # (34, 1, 8, 8, 6)

        return output


class SNeRVUpsampler(nn.Module):
    """
    SNeRV Upsampler. It combined the upsampling together with the encoding and cropping.
    """
    ...
    def forward(self, x: torch.Tensor, idx: torch.IntTensor, idx_max: tuple[int, int, int],
        ...
        # print("Upsmaple forward")
        # print("scale:", scale)
        # print("idx_max", idx_max)
        # print("size, padding", size, padding)
        # print("N, T_in, H_in, W_in, C", x.shape)
        ...
        # print("T_scale, H_scale, W_scale", T_scale, H_scale, W_scale)
        # print("T_out, H_out, W_out", T_out, H_out, W_out)
...
class SNeRVDecoder(nn.Module):
    ...
    def __init__(self, logger, input_size: tuple[int, int, int]=(600, 9, 16), input_channels: int=128, 
        ...
        for i, scale in enumerate(self.scales):
            ...
            self.blocks[-1].append(SNeRVUpsampler(**_upsample_cfg))
            ...
            if i == 1 or i == 2:
                # head block for low resolution
                _low_head_cfg = cfg_override(head_cfg, C1=C2, C2=self.output_channels,
                                        Ch=max(C2, self.output_channels),
                                        kernel_size=1)
                self.low_heads.append(get_block(**_low_head_cfg)) 
    ...
    def forward(self, x: torch.Tensor, idx: torch.IntTensor, idx_max: tuple[int, int, int], patch_mode: bool=True):
        ...
        for i, block in enumerate(self.blocks):
            ...
            # Run block for 1080p
            for j, layer in enumerate(block):    # depth 8, 8, 8, 8

                if isinstance(layer, SNeRVUpsampler):
            ...
            # Run block for middle variable 270p
            for j, layer in enumerate(block):
                if (i == 0 and j > 5) or (i == 1 and j > 5) or (i > 1):    # depth 4, 4
                    continue
                # print("270p block:", i, "index:", j)
                if isinstance(layer, SNeRVUpsampler):
                    x1 = layer(x1, idx=idx, idx_max=idx_max,
                            size=v_size_out,
                            scale=scale,
                            padding=padding,
                            patch_mode=patch_mode,
                    ...
                    x1 = layer(x1, px_mask_3d)
            
                if i == 1 and j == 5:
                    y_270p = self.low_heads[0](x1)
                    y_270p = crop_tensor_nthwc(y_270p, p_size_out)
                    # print("y_270p.shape", y_270p.shape) # N, T, H2, W2, C       

            # Run block for middle variable 540p
            for j, layer in enumerate(block):
                if (i == 0 and j > 7) or (i == 1 and j > 7) or (i == 2 and j > 7) or (i > 2):    # depth 6, 6, 6
                    continue
                # print("540p block:", i, "index:", j)

                if isinstance(layer, SNeRVUpsampler):
                    x2 = layer(x2, idx=idx, idx_max=idx_max,
                            size=v_size_out,
                            scale=scale,
                            padding=padding,
                            patch_mode=patch_mode,
                    ...
                    x2 = layer(x2, px_mask_3d)

                if i == 2 and j == 7:
                    # assert y_270p is not None
                    # y_up270p = upscale_resolution(y_270p, scale)
                    # y_up270p = crop_tensor_nthwc(y_up270p, p_size_out)

                    y_540p = self.low_heads[1](x2)
                    y_540p = crop_tensor_nthwc(y_540p, p_size_out)

                    # y_540p = y_up270p + y_540p
                    # print("y_540p.shape", y_540p.shape) # N, T, H2, W2, C       

        # # upscale to 1080p
        # assert y_540p is not None
        # y_up540p = upscale_resolution(y_540p, scale)
        # y_up540p = crop_tensor_nthwc(y_up540p, p_size_out)
            
        x = self.head(x)
        y_1080p = crop_tensor_nthwc(x, p_size_out)
        # y_1080p = y_up540p + y_1080p
        assert_shape(y_1080p, (idx.shape[0],) + p_size_out + (self.output_channels,))

        return y_1080p, y_270p, y_540p 
...
class SNeRV(nn.Module):
    ...
    def forward(self, input):
        ...
        # print("start encoding idx:", idx)
        output = self.encoding(idx, idx_max, padding=input_padding)
        # print("encoding output.shape", output.shape)
        output_1080p, output_270p, output_540p = self.decoder(output, idx, idx_max, patch_mode=patch_mode)
        # print("decoding output.shape", output.shape)

        # Reshape output
        if not self.training and self.eval_patch_size is not None :
            output_1080p = patch_to_video(output_1080p, input['patch_size'])
        output_1080p = output_1080p.permute(0, 4, 1, 2, 3).contiguous(memory_format=torch.channels_last_3d)   # N, T, C, H, W
        output_540p = output_540p.permute(0, 4, 1, 2, 3).contiguous(memory_format=torch.channels_last_3d)
        output_270p = output_270p.permute(0, 4, 1, 2, 3).contiguous(memory_format=torch.channels_last_3d)
...
def build_encoding(args, logger, size, channels):
    ...
    # Bulid encoding
    logger.info(f'Building SNeRV Encoding with cfg: {cfg}')
    encoding = SNeRVEncoding(**cfg)
...
def build_decoder(args, logger, input_size, input_channels, output_size, output_channels):
    ...
    # Bulid decoder
    logger.info(f'Building SNeRV Decoder with cfg: {cfg}')
    decoder = SNeRVDecoder(logger, **cfg)
...
def build_model(args, logger, input):
    ...
    # TODO: change output_size after upscaling
    # upscaling_factor = 2
    # output_size = (input['video_size'][0], input['video_size'][1], input['video_size'][2])
    output_size = (input['video_size'][0], input['video_size'][1], input['video_size'][2])
    ...
    decoder = build_decoder(args, logger, args.base_size, args.base_channels, output_size, 3)  # TODO: upscaling

    # Building model wrapper
    wrapper_cfg = {}
    wrapper_cfg['eval_patch_size'] = args.eval_patch_size

    logger.info(f'Building SNeRV with cfg: {wrapper_cfg}')

    model = SNeRV(encoding, decoder, **wrapper_cfg)

    return model


def set_args(parser):
    # SNeRV architecture parameters
    group = parser.add_argument_group('Model specific parameters')

    # Model complexity
    group.add_argument(f'--channels', type=int, default=256, help='channels of SNeRV')
    group.add_argument(f'--channels-reduce', type=float, default=2.0, help='channels reduction factor of SNeRV')
    group.add_argument(f'--channels-reduce-base', type=int, default=1, help='the base number which force to be a factor of channels of SNeRV')
    group.add_argument(f'--channels-min', type=int, default=1, help='min channels of SNeRV')
    group.add_argument(f'--depths', type=int, nargs='+', default=[3, 3, 3, 1], help='depths of SNeRV')
    group.add_argument(f'--exps', type=float, nargs='+', default=[4., 4., 4., 1.], help='expansion ratio of SNeRV')
    group.add_argument(f'--stem-kernels', type=int, default=1, help='stem kernel sizes of SNeRV')
    group.add_argument(f'--kernels', type=int, nargs='+', default=[1, 1, 1, 1], help='kernel sizes of SNeRV')
    group.add_argument(f'--scales-t', type=int, nargs='+', default=[1, 1, 1, 1], help='scaling factors in temporal dimension of SNeRV')
    group.add_argument(f'--scales-hw', type=int, nargs='+', default=[5, 4, 3, 2], help='scaling factors in spatial dimensions of SNeRV')
    group.add_argument(f'--stem-paddings', type=int, nargs='+', default=[0, 1, 1], help='stem paddings of SNeRV')
    group.add_argument(f'--paddings', type=int, nargs='+', default=[0, 1, 1], help='paddings of SNeRV')
    
    # Base
    for prefix in ['base']:
        group.add_argument(f'--{prefix}-size', type=int, nargs='+', default=[600, 9, 16], help='encoding size of SNeRV')
    ...
    # Upsample layers
    group.add_argument('--upsample-type', type=str, default='trilinear', help='Upsampling method of SNeRV')
    group.add_argument('--upsample-config', type=str, default='matmul-th-w', help='Upsampling method config of SNeRV')
    ...
    # Wrapper
    group.add_argument('--eval-patch-size', type=int, nargs='+', default=None, help='patch size during evalution for SNeRV')