"""
SNeRV Training Script
"""

from utils import *
from datasets import create_multivideo_dataset, create_loader, set_dataset_args
from snerv_tasks import MultiVideoRegressionTask, set_task_args
from snerv_compress import *
...
group.add_argument('--model', default='SNeRV', type=str, help='Model to build, e.g. SNeRV.')
...
group.add_argument('--trained_dict', default=None, type=str, help='Checkpoint path for trained state dict (from low resolution training).')
...
def train_step(args, logger, suffix, epoch, model, loader, optimizer, scheduler, task, accelerator, log_output):
    ...
    for i, batch in enumerate(loader):
        ...
        with accelerator.accumulate(model):
            inputs, _, outputs_1080p, outputs_270p, outputs_540p, loss, metrics = task.step(model, loader, batch)
            mean_loss = loss.mean()
        ...
        # Logging outputs
        if log_output:
            task.log_eval(dir_name=f'{epoch}_{suffix.lower()}', inputs=inputs, outputs_1080p=outputs_1080p, outputs_270p=outputs_270p, outputs_540p=outputs_540p, metrics=metrics) 
...
def eval_step(args, logger, suffix, epoch, model, loader, task, accelerator, log_output):
    ...
    for i, batch in enumerate(loader):
        ...
        # Logging outputs
        if log_output:
            task.log_eval(dir_name=f'{epoch}' + ('' if not suffix else f'_{suffix.lower()}'), inputs=inputs, outputs_1080p=outputs_1080p, outputs_270p=outputs_270p, outputs_540p=outputs_540p, metrics=metrics)
...
def main():
    ...
    # Task setting
    train_task = MultiVideoRegressionTask(args, logger, accelerator, root=os.path.join(output_dir, 'train_output'), 
                                     training=True, enable_log_eval=False)
    eval_task = MultiVideoRegressionTask(args, logger, accelerator, root=os.path.join(output_dir, 'eval_output'),
    ...
    train_dataset = create_multivideo_dataset(logger=logger, args=args, training=True)
    ...
    eval_dataset = create_multivideo_dataset(logger=logger, args=args, training=False)
    ...
    # Create model for 270p (4x), 540p (2x)
    logger.info(f'Create model for 270p, 540p:')
    args_2x = args
    args_2x.depths = [6, 6, 6]
    args_2x.exps = args.exps[:3]
    args_2x.kernels = args.kernels[:3]
    args_2x.scales_t = args.scales_t[:3]
    args_2x.scales_hw = args.scales_hw[:3]
    input  = train_task.parse_input(train_loader, next(iter(train_loader)))[0]
    input['video_size'] = (input['video_size'][0], input['video_size'][1]/2, input['video_size'][2]/2)
    model_2x = model_cls['SNeRV'].build_model(args_2x, logger, input)
    logger.info('Created model for 540p.')
    # logger.info(model_2x)

    args_4x = args
    args_4x.depths = [4, 4]
    args_4x.exps = args.exps[:2]
    args_4x.kernels = args.kernels[:2]
    args_4x.scales_t = args.scales_t[:2]
    args_4x.scales_hw = args.scales_hw[:2]
    input['video_size'] = (input['video_size'][0], input['video_size'][1]/2, input['video_size'][2]/2)
    model_4x = model_cls['SNeRV'].build_model(args_4x, logger, input)
    logger.info(f'Created model for 270p.')
    # logger.info(model_4x)
    ...
    # Restore trained state from low resolution
    if args.trained_dict is not None:
        model = compression.utils.unwrap_model(model)
        logger.info(f'Restore from trained state dict: {args.trained_dict}')
        weights_name = f"pytorch_model.bin"
        input_model_file = os.path.join(args.trained_dict, weights_name)
        loaded_state_dict = torch.load(input_model_file)
        ...
        for name, param in loaded_state_dict.items():
            if name in model_state_dict:
                if model_state_dict[name].shape == param.shape:
                    ...
                    logger.info(f"Shape mismatch for {name}: model shape {model_state_dict[name].shape}, "
                            f"loaded shape {param.shape}. Skipping...")
            else:
                logger.info(f"Parameter {name} not found in the model. Skipping...")
        model.load_state_dict(new_state_dict, strict=False)
        # Freeze the matched parameters
        for name, param in model.named_parameters():
            if name in loaded_state_dict and 'head' not in name and loaded_state_dict[name].shape == param.shape:
                param.requires_grad = False
                logger.info(f"Parameter {name} has been frozen.")
            else:
                logger.info(f"Parameter {name} is not frozen.")
        ...
        while epoch < args.epochs + args.prune_epochs + args.quant_epochs:
            ...
            if do_eval(args, quant_epoch, args.quant_epochs):
                ...
                for quant_level in sorted(args.quant_level, reverse=True):
                    ...
                    bits_per_pixel = num_bytes * 8 / np.prod(eval_dataset.video_size)
                    bitrate = num_bytes * 8 / (5)   # num_bits /(5 seconds)

                    logger.info(f'1080p Compressed model size: {num_bytes / 10**6:.2f}MB')
                    logger.info(f'1080p Bits Per Pixel (BPP): {bits_per_pixel:.4f}')
                    logger.info(f'1080p Bitrate (kbps): {bitrate/ 10**3:.4f}')

                    num_bytes_2x = compress_sub_bitstream(args, logger, accelerator, model_2x, model, '2x', os.path.join(output_dir, 'bitstreams_2x'), quant_level)
                    bits_per_pixel_2x = num_bytes_2x * 8 / (np.prod(eval_dataset.video_size)/4)
                    bitrate_2x = num_bytes_2x * 8 / (5)   # num_bits /(5 seconds)

                    logger.info(f'540p Compressed model size: {num_bytes_2x / 10**6:.2f}MB')
                    logger.info(f'540p Bits Per Pixel (BPP): {bits_per_pixel_2x:.4f}')
                    logger.info(f'540p Bitrate (kbps): {bitrate_2x/ 10**3:.4f}')

                    num_bytes_4x = compress_sub_bitstream(args, logger, accelerator, model_4x, model, '4x', os.path.join(output_dir, 'bitstream_4x'), quant_level)
                    bits_per_pixel_4x = num_bytes_4x * 8 / (np.prod(eval_dataset.video_size)/16)
                    bitrate_4x = num_bytes_4x * 8 / (5)   # num_bits /(5 seconds)

                    logger.info(f'270p Compressed model size: {num_bytes_4x / 10**6:.2f}MB')
                    logger.info(f'270p Bits Per Pixel (BPP): {bits_per_pixel_4x:.4f}')
                    logger.info(f'270p Bitrate (kbps): {bitrate_4x/ 10**3:.4f}')
