...
def compress_sub_bitstream(args, logger, accelerator, model, whole_model, quality, output_dir, quant_level):
    model = compression.utils.unwrap_model(model)
    whole_model = compression.utils.unwrap_model(whole_model)

    model_state_dict =  model.state_dict()
    new_state_dict = {}
    for whole_name, param in whole_model.named_parameters():
        name = whole_name.replace(".parametrizations.weight.original",".weight")
        if name in model_state_dict and 'head' not in name:
            if model_state_dict[name].shape == param.shape:
                new_state_dict[name] = param
        elif f'head_{quality}' in name:
            subname = name.replace(f".head_{quality}.", ".head.")
            if subname in model_state_dict and model_state_dict[subname].shape == param.shape:
                new_state_dict[subname] = param
